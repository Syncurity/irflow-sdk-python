...
_NOTE: IR-Flow ships with a built in account named api, and a built in group named api. The api_
_user is a member of the api group, and the api group has api read and write permissions. This is_
_the normal user used for api calls, though there are plenty of use cases for creating_
_additional/different users with varying permissions._
...
_NOTE: The log screen is very wide sometimes, and you may need to scroll to the right to see the IP Address._
...
A template is provided for you named api.conf.template. Make copy with the following command:
...
## A Couple Comments about using the irflow_client
...
- You use the instantiated object to make calls to the IR-Flow REST API without worrying about the headers,
gets and put, and the details of the connection.
